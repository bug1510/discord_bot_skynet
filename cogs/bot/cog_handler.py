...
class CogHandler(commands.Cog):
    ...
    async def init_cogs(self, path: str, file_prefix: str, extensions: list = ['']) -> None:
        #this loop collects all modules in cogs and put them in initial_extension
        for CogFile in os.listdir(self.bot.source + path):
            if CogFile.endswith('.py'):
                cog = file_prefix + CogFile[:-3]
                if cog not in extensions:
                    extensions.append(cog)
                    #self.bot.logger.info('append cog - ' + module)
    ...
    # async def list_available_cogs(self):
    #     ListModulesField = ''
    ...
    # async def unload_cogs(self, cogs):
    #     extension_path = 'cogs.' + extension
    #     member = ctx.message.author
